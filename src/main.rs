...
// use bip39::Mnemonic;
// use bitcoin::{
//     bip32::{DerivationPath, ExtendedPrivKey}, key::Secp256k1, secp256k1::{Keypair, SecretKey, XOnlyPublicKey}, Address, Network, PrivateKey, ScriptBuf
// };
// use rand::Rng;
// use serde::Deserialize;
...
//         let mnemonic = generate_mnemonic();
//         println!("Mnemonic: {}\n", mnemonic);
//         println!("Bitcoin Taproot Wallets:");
        
//         for i in 0..2 {
//             // Use purpose 86' for Taproot
//             let path = DerivationPath::from_str(&format!("m/86'/0'/0'/0/{}", i))
//                 .expect("Failed to parse derivation path");
...
//             // Inside the main loop, replace the Taproot generation code with:
//             let keypair = Keypair::from_secret_key(&secp, &secret_key);
//             let (x_only_pub_key, _parity) = XOnlyPublicKey::from_keypair(&keypair);
                        
//             // Create Taproot address
//             let tr_address = Address::p2tr(&secp, x_only_pub_key, None, Network::Bitcoin);
            
//             println!("{:2}. Address: {}", i + 1, tr_address);
//             println!("    View on BlockExplorer: https://blockexplorer.one/bitcoin/mainnet/address/{}", tr_address);
//             println!("    Private key (WIF): {}", priv_key.to_wif());
            
//             thread::sleep(Duration::from_millis(100));

//             let url = format!("https://blockchain.info/balance?active={}", tr_address);
//             let balance_btc = match reqwest::blocking::get(&url) {
//                 Ok(resp) => {
//                     if resp.status().is_success() {
//                         match resp.text() {
...
//                                 match serde_json::from_str::<BalanceResponse>(&text) {
//                                     Ok(balance_info) => {
//                                         match balance_info.addresses.get(&tr_address.to_string()) {
//                                             Some(addr_info) => addr_info.final_balance as f64 / 100_000_000.0,
//                                             None => 0.0,
//                                         }
//                                     },
